...
const ApiContextProvider = (props) => {
  ...
  const newVideo = async () => {
    ...
    uploadDate.append("title", title);
    uploadDate.append("video", video, video.name);
    uploadDate.append("thum", thum, thum.name);
    try {
  ...
  const incrementLike = async () => {
    try {
      const uploadDate = new FormData();
      uploadDate.append("like", selectedVideo.like + 1);
  ...
  const incrementDislike = async () => {
    try {
      const uploadDate = new FormData();
      uploadDate.append("dislike", selectedVideo.dislike + 1);
